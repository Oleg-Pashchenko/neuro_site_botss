...
def get_annotation(pipeline) -> str:
    ...
    cur = conn.cursor()
    cur.execute("SELECT text FROM public.users_application_amocrm_pipelines WHERE id=%s", (pipeline,))
    resp = cur.fetchone()
    conn.close()
    return resp[0]
...
def get_params(pipeline):
    ...
    cur = conn.cursor()
    cur.execute("SELECT * FROM public.users_application_amocrm_pipelines WHERE id=%s", (pipeline,))
    resp = cur.fetchone()
    conn.close()
    return resp
